...
本格実装のフェーズ 1〜5 まで完了済み（2026-07-04）。残作業はフェーズ 5 の後に記載。
それ以前の予備検証（最小イメージ動作確認、コンテナ内での ninja video 通し確認）も完了済み。
...
- ホスト実行用（`share/rules.ninja`、`templates/default/`、`bin/*.ros`、`bin/*.sh`）は
  一切変更しない。
- podman 用は別ファイル: `share/rules-podman.ninja`、`templates/podman/`。
- bin のスクリプトに修正が必要になった場合も、`abc.ros` なら `abc-podman.ros`、
  `abc.sh` なら `abc-podman.sh` のようにコピーしてから直す（冗長でもよい）。
- **init の名前は 2026-07-04 に主客を入れ替えた**: 標準の `bin/init.sh` が podman 版
  （旧 init-podman.sh）、ホスト実行版は `bin/init-host.sh`（旧 init.sh）。
  実装フェーズ 1〜3 の記録は当時の名前（init-podman.sh）のまま。
- 試験は .gitignore 済みの `podman/work2/` で行う。markdown-cast への symlink を置き、

  ```sh
  cd podman/work2
  sh markdown-cast/bin/init.sh <NAME>
  cd <NAME> && ninja video
  ```
...
- `test/podman/01-init-podman.test`: podman 版 init の足場生成から
...
### フェーズ 5: 混在検証 -- 完了（2026-07-04）

- ホスト実行の deck と podman の deck を同居させ、それぞれ独立して ninja が動くことを
  ユーザーの環境で確認済み。

### 残作業

- Apple Silicon（podman machine 経由）での実機確認。
- イメージの ghcr.io への push（ユーザーが実施予定）と、push 後の
  init.sh の pull 対応（TODO.md 参照）。
...
`init.sh`（podman 版）が生成する `build.ninja` はこれを include する
...
## init.sh（podman 版）の役割

1. podman がなければエラーで止まる（init-host.sh の案内を表示）。
2. イメージが未ビルドなら `podman build` を走らせる。
3. `templates/podman/` から `build.ninja` を生成する（rules-podman.ninja を include）。
4. `init-host.sh` はホスト実行版の build.ninja を生成する（旧 init.sh。処理は無変更）。
